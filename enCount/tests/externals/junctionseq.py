...
junctionseq.OPEN = Mock(return_value=None)
...
class TestQoRTs(unittest.TestCase):
    ...
    # TODO: include /QoRTsPipelineWalkthrough/outputData/JctSeqData/inst/extdata/tiny
    ...
    def test_QoRTs(self):
        ...
        if exists(self.in_decoder_uid):
            reader = DictReader(open(self.in_decoder_uid, "rt"), delimiter="\t")
            for row in reader:
                ...
                r = junctionseq.run_QoRTs_count(in_bam=in_bam, in_gtf=self.in_gtf,
                                out_dir=out_bam_accession, test_run=True)
                self.assertEqual(r, 0)
        else:
            out_bam_accession = join(self.out_dir_raw_cts, "test_bam")
            r = junctionseq.run_QoRTs_count(in_bam="in_bam", in_gtf=self.in_gtf,
                                            out_dir=out_bam_accession, test_run=True)
            self.assertEqual(r, 0)

        # Merge counts for technical replicates """
        r = junctionseq.run_QoRTs_merge_counts(in_dir=self.out_dir_raw_cts,
                        in_decoder=self.in_decoder_uid,
                        out_dir=self.out_dir_cts)
        self.assertEqual(r, 0)

        # Estimate size factors
        r = junctionseq.run_QoRTs_size_factors(in_dir=self.out_dir_raw_cts,
                              in_decoder=self.in_decoder_uid,
        ...
        r = junctionseq.run_QoRTs_novel_splices(in_dir=self.out_dir_cts,
                               in_gtf=self.in_gtf,
        ...
        r = junctionseq.run_JunctionSeq_analysis(in_gtf_dir=self.in_count_dir,
                                     in_gff=self.in_gff,
